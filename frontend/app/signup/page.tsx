...
export default function SignUpPage() {
  ...
  async function onSignUp() {
    ...
    if (data.user && data.session) {
      setSessionUser({ id: data.user.id, email: data.user.email ?? email.trim() });
      router.push("/translate");
    }
  }

  async function signUpWith(provider: "google" | "github") {
        ...
        {!confirmed ? (
          <Card className="space-y-4 p-7">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => void signUpWith("google")} disabled={busy} variant="secondary">
                <span className="flex items-center justify-center gap-2">
                  {oauthLoading === "google" ? <Spinner /> : <GoogleIcon />}
                  {oauthLoading === "google" ? "Connecting..." : "Google"}
                </span>
              </Button>
              <Button onClick={() => void signUpWith("github")} disabled={busy} variant="secondary">
                <span className="flex items-center justify-center gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">or</span>
              <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800" />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") void onSignUp(); }}
              />
            </div>
            ...
            <Button onClick={() => void onSignUp()} disabled={busy} className="w-full">
              {loading ? (
